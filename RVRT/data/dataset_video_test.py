...
class SingleVideoRecurrentTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        scan_top_only = opt.get('scan_top_only', False)
        if scan_top_only:
            # Directly scan files in the top directory
            img_paths_lq = sorted(list(utils_video.scandir(self.lq_root, full_path=True)))
            subfolder_name = osp.basename(self.lq_root)
