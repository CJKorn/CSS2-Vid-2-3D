...
def infer(args, dataroot, outputdir):
    ...
    # parser.add_argument('--input', type=str, default='testsets/REDS4/sharp_bicubic',
    ...
    test_set = SingleVideoRecurrentTestDataset({
        ...
        'num_frame': -1, 
        'cache_data': False,
        'scan_top_only': not getattr(args, 'scan_subdirectories', False)  # Inverted logic
    })
...
def prepare_model_dataset(args):
    ...
    # download model
    model_path = os.path.join(args.model, f'{args.task}.pth')
    ...
    if os.path.exists(f'{args.temp}'):
        print(f'using dataset from {args.temp}')
    else:
