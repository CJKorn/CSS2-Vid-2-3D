...
from FMANet.model import FMANet
from FMANet.train import Trainer
from FMANet.utils import Report
from FMANet.data import get_dataset
from FMANet.config import Config
...
def test_custom(config, args):
    ...
    model = FMANet(config=config)
    trainer = Trainer(config=config, model=model, args=args)
    trainer.load_best_model()
    trainer.test(test_dataloader)
